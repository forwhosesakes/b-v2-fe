  ...
  return (
    ...
    <div className={`absolute`} style={{inset:props.report?.villa_style_loc}}>
      <div className="relative ">
        ...
        {props.report &&props.isSelected&&<><div className="popup-info absolute inset-0 flex flex-col gap-y-2  p-5 text-[8px]">
          <p className="text-nowrap  flex w-full items-center gap-x-2 text-xs">
            <img width={16} height={16} src="/assets/icons/house-rounded" />
            <span> فيلا {(props.report.id % 10) + 1} </span>{" "}
          </p>
          <p>{props.report.villa_style_loc}</p>
       ...
       {props.report&&props.isSelected&& <svg
      ...
      <svg
        onClick={()=>{
          ...
          className={`relative  ${props.isSelected?'opacity-100':'opacity-0 hover:opacity-100 hover:fill-pink-300'} right-8 cursor-pointer`}
        >
        
          <path
            d="M0 12.2523L1.52381 0L16 0.429907V23H0V15.2617V12.2523Z"
            fill={props.isSelected? randomColor:''}
            fillOpacity="0.5"
          />
        </svg></>}
      </div>
